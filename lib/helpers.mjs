...
const helpers = {
  ...
  sendTwilioSMS: (phone, msg, callback) => {
    ...
    msg = typeof (msg) === 'string' && msg.length > 0 && msg.length <= 1600 ? msg.trim().replace(/ /g, "%") : false
    if (phone && msg) {
      ...
      const requestDetail = {
        ...
        'path': `/sms/1/text/query?username=${environmentToExport.twilio.username}&password=${environmentToExport.twilio.password}&from=${environmentToExport.twilio.fromPhone}&to=${phone}&text=${msg}!!!`,
        'headers': {
