...
handlers._users = {
  ...
  delete: (data,callback)=>{
    ...
    var phone = typeof(data.queryStringObject.phone) == 'string'? data.queryStringObject.phone.trim() : false;
    if(phone){
      ...
      helpers.verifyToken(token,phone,function(tokenIsValid){
        if(tokenIsValid){
          // Lookup the user
                ...
                if(!err){
                  ...
                  if(checksToDelete > 0){
                    ...
                    userChecks.forEach(function(checkId){
                      // Delete the check
                      lib.delete('checks',checkId,function(err){
                        if(err){
                          deletionErrors = true;
                        }
                        checksDeleted++;
      ...
      if (protocol || url || method || successCodes || timeoutSeconds) {
        ...
        lib.read('checks', id, function (err, checkData) {
          if (!err && checkData) {
            ...
            helpers.verifyToken(token, checkData.userPhone, function (tokenIsValid) {
              if (tokenIsValid) {
                ...
                lib.update('checks', id, checkData, function (err) {
                  if (!err) {
                    callback(200);
                  } else {
          ...
          helpers.verifyToken(token, checkData.userPhone, function (tokenIsValid) {
            if (tokenIsValid) {
              ...
              lib.delete('checks', id, function (err) {
                if (!err) {
                  lib.read('users', checkData.userPhone, (err, userData) => {
                    if (!err && userData) {
                      const findIndex = userData.checks.findIndex(check => check.id == id)
                      userData.checks.splice(findIndex, 1)

                      lib.update('users', checkData.userPhone, userData, (err, userData) => {
                        if (!err) {
                          callback(200);
                        } else {
